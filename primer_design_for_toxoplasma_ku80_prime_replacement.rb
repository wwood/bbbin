...
if $0 == __FILE__
  ...
  #
  # Are unique restriction sites within restricition site-800bp-500bp to 3' end of
  # sequence?
  # If no, then they are of no use, discard that restriction site.
  # This lambda takes an array of EnzymeCut objects, and returns those that are suitable
  remove_unsuitable_enzymes = lambda do |enzyme_cut_array|
    ...
    enzyme_cut_array.select do |c|
      ...
      if beginner <= 0 #this hopefully won't happen much for restriction sites that
        ...
        # Enzyme cut sites are too close to the 3' end. Need enough homolgous sequence.
        false
      else
