        ...
        for (accession, tax) in zip(df.accession, df.gtdb_taxonomy):
            ...
            if a2 in l.name:
                if taxonomy is not None:
                    raise Exception("The leaf {} matched to 2+ different taxonomy strings".format(l.name))
                taxonomy = tax
        if taxonomy is None:
            failed += 1
        else:
            print("{}\t{}".format(l.name, taxonomy))
    
    if failed > 0:
        logging.warn("Failed to find taxonomy for {} leaves".format(failed))
